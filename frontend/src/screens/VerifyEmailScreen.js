...
export default function VerifyEmailScreen({ route, navigation }) {
    ...
    const handleVerify = async (enteredCode) => {
        ...
        setLoading(true);
        try {
            const res = await authApi.verifyEmail({ userId, code: enteredCode || code });
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            setError(e?.response?.data?.message || 'Could not resend. Please try again.');
        }
    };
...
const styles = StyleSheet.create({
    ...
    subText: {
        ...
        fontSize: 12, color: '#444', lineHeight: 18, marginBottom: 4,
    },
